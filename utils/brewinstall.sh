...
if grep -E -w 'kernel-rt' <<<"${builds[*]}"; then
	...
	if [[ "$FLAG" == debugkernel || -n "${DEBUG_INFO_OPT}" ]]; then #<-assume install kernel-rt from other site
		# current only install dependency packages.
		run "yum install -y --nogpgcheck --setopt=keepcache=1 --skip-broken kernel-rt-debug --exclude=kernel-rt" -
	else
		# only rtk
	...
	if [[ "$FLAG" == debugkernel || -n "${DEBUG_INFO_OPT}" ]]; then
		run "yum install -y --nogpgcheck --setopt=keepcache=1 --skip-broken kernel-64k-debug --exclude=kernel-64k" -
	else
		[[ ${INSTALL_BOOTC} == 'yes' ]] && clean_old_kernel
